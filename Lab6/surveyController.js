...
module.exports = function(app){
    ...
    app.get('/analysis', function(req, res){
        var use = readData("use");
        var name = readData("name");
        var email = readData("email");
        var freq = readData("freq");
        var feedback = readData("feedback");
        var engine = readData("engine");
        res.render('showResults', {results: [engine, use, name, email, freq, feedback]});
        console.log([engine, use, name, email, freq, feedback]);
    });
    ...
    app.post('/niceSurvey', urlencodedParser, function(req, res){
        ...
        for (var key in json){
            ...
            if ((key === "engine") && (json[key].length === 0)) {
